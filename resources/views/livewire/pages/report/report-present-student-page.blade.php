                    ...
                    <td class="px-6 py-4 text-center">
                        @if (!empty($item['special_day']))
                            <span
                                ...
                                {{ ucfirst(str_replace('_', ' ', $item['special_day'])) }}
                            </span>
                        @else
                            @if($item['status'] == 'H') 
                                <span class="text-gray-500 dark:text-gray-300 font-bold">{{ $item['status'] }}</span>
                            @elseif($item['status'] == 'S') 
                                <span class="text-green-500 font-bold">{{ $item['status'] }}</span>
                            @elseif($item['status'] == 'I') 
                                <span class="text-purple-500 font-bold">{{ $item['status'] }}</span>
                            @elseif($item['status'] == 'A') 
                                <span class="text-red-500 font-bold">{{ $item['status'] }}</span>
                            @elseif($item['status'] != '-') 
                                <span class="text-orange-500 font-bold">{{ $item['status'] }}</span>
                            @else 
                                {{ $item['status'] }}
                            @endif
                        @endif
